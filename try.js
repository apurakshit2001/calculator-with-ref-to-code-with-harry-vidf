...
Array.from(buttons).forEach((button) => {
    button.addEventListener('click', (abcd) => {
        ...
        if (abcd.target.innerHTML == '=') {
            ...
            // if (abcd.target.innerHTML == '+' || abcd.target.innerHTML == '-' || abcd.target.innerHTML == '*' || abcd.target.innerHTML == '/') {
            //     let operator = abcd.target.innerHTML;
            //     let array = ab.split(operator);
            ...
            console.log(string, document.querySelector('input').value.replace(positionValue, string1))
            console.log(console.log(string, document.querySelector('input').value.replace(positionValue, string1)))
